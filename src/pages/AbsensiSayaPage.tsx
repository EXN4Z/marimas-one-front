...
import { getAbsensiSaya, scanAbsen, type Karyawan, type Absensi } from '../api/absensi';
...
export default function AbsensiSayaPage() {
  ...
  const konfirmasi = async () => {
    ...
    setSubmitError('');
    try {
      const result = await scanAbsen(capturedPhoto, gps.lat, gps.lng, faceResult.verified, faceResult.distance);
    ...
    <AppLayout title="Absensi">
      <div className="max-w-md mx-auto">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 text-center">
          ...
          <p className="text-sm text-slate-400 mb-6">{pekerja.nip}</p>
