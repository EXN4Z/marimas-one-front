...
import { FileSpreadsheet, ClipboardList, PackageSearch, Download, Loader2 } from 'lucide-react';
import AppLayout from '../components/AppLayout';
import { useAuth } from '../context/AuthContext';
import { downloadLaporanAbsensi, downloadLaporanIzin, downloadLaporanInventaris } from '../api/laporan';
...
export default function Laporan() {
  ...
  const [downloadingKey, setDownloadingKey] = useState<LaporanKey | null>(null);
  const [error, setError] = useState('');

  const handleDownload = async (key: LaporanKey) => {
    setDownloadingKey(key);
    setError('');
    try {
      await laporanConfig[key].download(bulan, tahun);
    } catch (err: any) {
      if (err.response?.status === 403) {
        setError('Anda tidak punya akses untuk mengunduh laporan ini.');
      } else {
        setError(`Gagal mengunduh ${laporanConfig[key].label.toLowerCase()}.`);
      }
    } finally {
    ...
    <AppLayout title="Laporan">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-6">
        <p className="text-sm text-slate-500 max-w-lg">
          Unduh rekap bulanan dalam format CSV, siap dibuka langsung di Excel.
        </p>
