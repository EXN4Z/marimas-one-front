...
// // BARU: role akun cabang. Cabang dapet sebagian section analytics (ringkasan
// // izin, distribusi karyawan per departemen, tren kehadiran mingguan, top
// // kehadiran, top pengajuan izin) TAPI di-scope cuma ke karyawan yang ada di
// // cabang tsb — dan TIDAK dapet hero chart "Pengajuan Izin Tahun Ini" maupun
// // section Inventaris.
// //
// // ⚠️ PENTING: scoping "cuma karyawan di cabang ini" itu tanggung jawab
// // BACKEND. Endpoint yang dipanggil (fetchStatsCard, fetchKehadiranMingguan,
// // fetchDepartemenDistribusi, fetchRingkasanIzin, fetchTopKehadiran,
// // fetchTopKaryawan) TETAP SAMA seperti punya admin — backend yang harus
// // mendeteksi role user login == 'cabang' dan filter query berdasarkan
// // cabang_id user tsb. Kalau backend belum difilter, akun cabang bakal
// // tetap lihat data SEMUA karyawan/cabang, bukan cuma cabangnya sendiri.
...
// // BARU: sebagian query di bawah (ringkasan izin, top kehadiran, top karyawan)
// // juga diaktifkan buat role 'cabang' — lihat CABANG_ROLE di atas.
...
//   // BARU: flag khusus akun cabang — lihat penjelasan CABANG_ROLE di atas.
//   const isCabang = cachedUser?.role === CABANG_ROLE;
...
//   // BARU: ringkasanIzin, topKehadiran, topKaryawan JUGA dinyalain buat akun
//   // cabang (isCabang) — 3 section ini yang diminta tampil di dashboard
//   // cabang. grafikPengajuan (hero chart) dan mutasiBarang/totalBarang
//   // (inventaris) TETAP cuma buat isApprover, gak dipakai di dashboard cabang.
//   const { data: ringkasanIzin } = useQuery({
//     queryKey: ['ringkasan-izin'],
//     queryFn: fetchRingkasanIzin,
//     enabled: sessionReady && (isApprover || isCabang),
//     staleTime: 2 * 60 * 1000,
//   });
...
//   // BARU: dashboard cabang dapet sebagian section analytics (ringkasan izin,
//   // top kehadiran, top pengajuan) tapi TIDAK dapet hero chart "Pengajuan
//   // Izin Tahun Ini" (itu murni buat isApprover).
//   const showRingkasanIzin = isApprover || isCabang;
//   const showTopSection = isApprover || isCabang;
...
//       {/* ==== Hero: tren pengajuan izin (chart utama, khusus approver) +
//           ringkasan status izin (approver & cabang). Layout-nya:
//           - isApprover: hero chart (span-2) + card ringkasan (span-1)
//           - isCabang doang (bukan approver): cuma card ringkasan sendirian ==== */}
//       {showRingkasanIzin && (
//         <div className={`grid grid-cols-1 ${isApprover ? 'xl:grid-cols-3' : ''} gap-6 mt-6`}>
//           {isApprover && (
...
//           <div className={`${cardClass} ${!isApprover ? 'lg:max-w-md' : ''}`}>
...
//           Ditampilin ke semua role (termasuk cabang) — backend yang scope
//           datanya berdasarkan cabang_id kalau rolenya 'cabang'. */}
...
//       {/* ==== Top kehadiran & top pengajuan (approver & cabang) ==== */}
//       {showTopSection && (
//         <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
//           <div className={cardClass}>
...
//       {/* ==== Inventaris (khusus approver — TIDAK ditampilin buat cabang) ==== */}
//       {isApprover && (
